...
function validateCustomerInfo() {
  ...
  if (!phoneRegex.test(phone)) {
    document.getElementById("phone").nextElementSibling.textContent = "Phone number should be exactly 10 digits.";
  ...
  const addressLine1Regex = /^[0-9\/]+$/;
  if (!addressLine1Regex.test(addressLine1)) {
    document.getElementById("address-line-1-error").textContent = "Write only house number.";
  ...
  const addressLine2Regex = /^[a-zA-Z]{10,}$/;
  if (addressLine2 !== "" && !addressLine2Regex.test(addressLine2)) {
    document.getElementById("address-line-2-error").textContent = "Address 2 should be at least 10 characters and contain only strings.";
  ...
  // Validate city
  const cityRegex = /^[a-zA-Z\s]{6,}$/;
  if (!cityRegex.test(city)) {
    document.getElementById("city-error").textContent = "City should be at least 6 characters and contain only alphabets.";
...
// Validate state
  const stateRegex = /^[a-zA-Z\s]{6,}$/;
  if (!stateRegex.test(state)) {
    document.getElementById("state-error").textContent = "State should be at least 6 characters and contain only alphabets.";
